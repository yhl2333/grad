...
class FanSeqChainConfig:
    ...
    reid_fuse_weight: float = 0.40,
    geom_fuse_weight: float = 0.60,
    ...
    for da in seq_a:
        ...
        for db, reid_score in topk_rows:
            ...
            if refine_score < cfg.min_refine_score:
                continue
            score_fused = (
                cfg.reid_fuse_weight * reid_score +
                (1.0 - cfg.reid_fuse_weight) * refine_score
            )
            ...
            cand = {
                ...
                "score_total": float(score_fused),    # 现在真正变成联合分数
...
if __name__ == "__main__":
    # -------- 输入 --------
    txt_a = "experient_fig/test/droneA_tracks.txt"
    txt_b = "experient_fig/test/droneB_tracks.txt"

    reid_npz_a = "experient_fig/test/droneA_reid.npz"
    reid_npz_b = "experient_fig/test/droneB_reid.npz"

    # Kalman sidecar（不开速度方向时可为 None）
    # kf_txt_a = None
    # kf_txt_b = None
    # 启用速度方向时改成：
    kf_txt_a = "experient_fig/test/droneA_kf.txt"
    kf_txt_b = "experient_fig/test/droneB_kf.txt"
    ...
    cfg = FanSeqChainConfig(
        ...
        refine_pos_weight=0.48,
        refine_area_weight=0.19,
        refine_h_weight=0.19,
        refine_dist_weight=0.14,
