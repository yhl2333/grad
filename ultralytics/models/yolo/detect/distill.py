...
class DetectionDistiller(BaseTrainer):
    ...
    def setup_model(self):
        """
        YOLO11-compatible setup_model for distillation
        - supports .pt (teacher / resume)
        - supports .yaml (student)
        """
        # 如果已经是 torch.nn.Module，直接返回（蒸馏中常见）
        ...
        # ===============================
        # 情况 1：传入的是 .pt（teacher / resume）
        # ===============================
        if isinstance(self.model, str) and self.model.endswith(".pt"):
            yolo = YOLO(self.model)
            self.model = yolo.model  # DetectionModel

            # ckpt 仅用于 resume / 兼容
            ckpt = getattr(yolo, "ckpt", None)

        # ===============================
        # 情况 2：传入的是 .yaml（student）
        # ===============================
        else:
            yolo = YOLO(self.model)
            self.model = yolo.model

            # 👉 如果你希望 student 加载部分预训练（结构允许）
            if getattr(self, "pretrain_weights", None):
                state = torch.load(self.pretrain_weights, map_location="cpu")
